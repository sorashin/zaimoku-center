...
3. 左メニュー「プロジェクトの設定 → スクリプト プロパティ」で登録:
   ...
   | プロパティ名 | 値 | 必須 |
   |---|---|---|
   | `NOTIFY_EMAIL` | 運営の通知先メール。カンマ区切りで複数可（例 `a@x.com, b@y.com`） | 任意（未設定なら運営メールは送らず蓄積のみ） |
   | `SHARED_TOKEN` | サイトの `GAS_WEBHOOK_TOKEN` と同じ任意の文字列（簡易認証用） | 推奨 |
   | `SHEET_NAME` | 書き込み先シート名（既定 `purchase_requests`） | 任意 |
   | `SITE_NAME` | 購入者宛メールの差出名（既定 `伊那森林素材センター ドバドバ`） | 任意 |

   購入者宛の「リクエストを承りました」自動返信は、サイトのフォームで入力された
   購入者メール宛に送られる（`NOTIFY_EMAIL` の設定有無に依らない）。
